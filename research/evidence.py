...
class ResearchEvidence:
    ...
    def __init__(self):
        self.blocked_words = {
            ...
            "archive", "archives", "search", "homepage",
            "home", "daily-ai-news"
        }
        self.weak_domains = {
    ...
    def has_date_signal(self, result):
        ...
        if date_value:
            return True
        text = str(result.get("content", "")) + " " + str(result.get("snippet", ""))
        patterns = [
    ...
    def score(self, result):
        ...
        for weak in self.weak_domains:
            if domain == weak or domain.endswith("." + weak):
                score -= 2
    ...
    def enrich(self, results):
        ...
        for result in results:
            item = dict(result)
            item["evidence_score"] = self.score(item)
            output.append(item)
        return output
    ...
    def filter(self, results):
        ...
        output = []
        for result in results:
            if self.is_bad_url(result.get("url", "")):
                continue
            if self.is_weak_title(result.get("title", "")):
                continue
